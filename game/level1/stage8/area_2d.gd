...
func _on_character_body_2d_new_area(area: Variant) -> void:
	...
	for i in get_children():
		if i.name.begins_with("Cell"):
			i.same_area = (cur_area == "Area2D2" or name == "Area2D2" or name == cur_area)
			i.queue_redraw()
	if cur_area == "Area2D2" or name == cur_area:
...
func _on_body_entered(body: Node2D) -> void:
	if body.name.begins_with("Cell"):
		...
		body.same_area = (cur_area == "Area2D2" or name == "Area2D2" or name == cur_area)
		body.queue_redraw()
		if name == "Area2D1":
